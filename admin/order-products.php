...
function order_products_media_url(?string $path): string
{
...
$pageTitle = 'Products - ' . $order['order_number'];
...
.order-product-card {
    ...
    border: 1px solid var(--detail-border);
    border-radius: 18px;
    box-shadow: 0 13px 34px rgba(65, 27, 38, 0.075);
}
...
.order-product-card-body p {
    margin: 3px 0;
    color: var(--detail-muted);
    font-size: 0.68rem;
}

.order-product-card-total {
    display: flex;
    align-items: center;
    justify-content: space-between;
    gap: 13px;
...
.order-product-card-total strong {
    color: var(--detail-primary);
    font-size: 0.88rem;
}

.order-product-view-link {
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 7px;
...
<div class="admin-detail-page">
    <div class="detail-topbar">
        <a
            href="order-view.php?id=<?= $orderId; ?>"
        ...
        <a href="orders.php" class="detail-action-link">
    ...
    <section class="detail-page-banner">
        <small>Separate ordered-products page</small>
        <h1>Products in <?= e($order['order_number']); ?></h1>
        <p>
            <?= count($items); ?> product lines ordered by
    ...
    <?php else: ?>
        <section class="order-product-catalogue">
            <?php foreach ($items as $item): ?>
                <article class="order-product-card">
                    ...
                    >
                        <img
                            src="<?= e(order_products_media_url(
                    ...
                    <div class="order-product-card-body">
                        <h2>
                            <?= e($item['product_name_snapshot']); ?>
                        </h2>

                        <?php if (!empty($item['sku_snapshot'])): ?>
                            <p>SKU: <?= e($item['sku_snapshot']); ?></p>
                        <?php endif; ?>
                        ...
                        <?php if (!empty(
                            $item['customer_item_notes']
                        )): ?>
                            <p>
                                Notes:
                                <?= e($item['customer_item_notes']); ?>
                            </p>
                        <?php endif; ?>

                        <div class="order-product-card-total">
                            <span>
                                Quantity:
                        ...
                        <?php if (!empty($item['product_id'])): ?>
                            <a
                                class="order-product-view-link"
                                href="product-view.php?id=<?= (int)$item['product_id']; ?>"
                            >
